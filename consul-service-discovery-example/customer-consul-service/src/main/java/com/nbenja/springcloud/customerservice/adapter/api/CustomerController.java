...
public class CustomerController {
    ...
    public ResponseEntity<List<Customer>> getAllCustomers(){
        return Optional.ofNullable(customerRepository.findAll()).map(customers -> ResponseEntity
                .status(HttpStatus.OK).body(customers)).orElse(ResponseEntity.status(HttpStatus
                .NOT_FOUND).body(null));
    }
